...
class ScrapeWorker(QThread):
    ...
    def _scrape_one(self, entry: RomEntry) -> bool:
        ...
        # Only include devid/devpassword if the user has registered ones
        if self.dev_id:
            params["devid"]       = self.dev_id
            params["devpassword"] = self.dev_pwd
        ...
        _try_download_media(entry, medias, "box-2D",     gamelist_dir, "images",      "image",      self.media_types)
        _try_download_media(entry, medias, "screenshot", gamelist_dir, "screenshots", "screenshot", self.media_types)
        _try_download_media(entry, medias, "wheel",      gamelist_dir, "marquees",    "marquee",    self.media_types)
        _try_download_media(entry, medias, "title-screen", gamelist_dir, "titlescreens", "titleshot", self.media_types)
...
class ScrapeDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        note = QLabel(
            ...
            'A free account is required. If you get 403 errors, verify your '
    ...
    def _on_start(self):
        ...
        self._worker = ScrapeWorker(self.entries, user, pwd, media_types)
