...
class FullDetailView(QWidget):
    ...
    def _render(self):
        ...
        # Launch — green, prominent
        lb = QPushButton("▶  Launch"); lb.setToolTip("Launch in RetroBat"); lb.setFixedHeight(32)
        lb.setStyleSheet(f"""
            QPushButton{{background:#0a2010;border:1px solid #1a6030;border-radius:5px;
                color:#40e080;font-size:13px;font-weight:bold;padding:0 18px;}}
            QPushButton:hover{{background:#0e3018;border-color:#30c060;color:#80ffa0;}}
        """)
        lb.clicked.connect(lambda: self.launch_requested.emit(entry))
        br.addWidget(lb)
        ...
        if entry.last_played: _mrow("Last Played", entry.last_played[:8])
        if entry.file_size_mb > 0: _mrow("File Size", f"{entry.file_size_mb:.1f} MB")

        # Backlog status
        if entry.backlog_status:
            status_labels = {"unplayed": "◌ Unplayed", "in_progress": "◑ In Progress", "completed": "◉ Completed"}
            _mrow("Status", status_labels.get(entry.backlog_status, entry.backlog_status))

        # Notes
        if entry.notes:
            _mrow("Notes", entry.notes[:120] + ("…" if len(entry.notes) > 120 else ""))

        # PDF manual button
        if entry.manual and Path(str(entry.manual)).exists() and str(entry.manual).lower().endswith(".pdf"):
            man_path = Path(str(entry.manual))
            man_btn = QPushButton("📄  Open Manual")
            man_btn.setFixedHeight(28)
            man_btn.setStyleSheet("""
                QPushButton{background:#0e2030;border:1px solid #2a4a60;border-radius:4px;
                    color:#60a0d0;font-size:11px;padding:0 10px;margin-top:6px;}
                QPushButton:hover{background:#162840;border-color:#3a80b0;color:#90d0ff;}
            """)
            def _open_manual(p=man_path):
            ...
            man_btn.clicked.connect(_open_manual)
            mcl.addWidget(man_btn)
