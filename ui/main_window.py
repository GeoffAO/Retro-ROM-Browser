...
from ..core.retrobat import RetroBatInstall, find_retrobat_installs
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._retrobat: Optional[RetroBatInstall] = None
    ...
    def _build_ui(self):
        ...
        self._rental_panel.rent_toggled.connect(self._on_rent_toggled)
        ...
        self._full_detail.launch_requested.connect(self._on_launch)
    ...
    def _init_retrobat(self):
        """Try to connect to the configured RetroBat install."""
        rb_path = self._settings.retrobat_path
        if not rb_path:
            # Auto-detect
            found = find_retrobat_installs()
            if found:
                rb_path = str(found[0].root)
                self._settings.retrobat_path = rb_path
                self._settings.import_ss_credentials_from_retrobat()
                self._settings.save()
        ...
        if rb_path:
            try:
                ...
                if rb.is_valid():
                    self._retrobat = rb
                    self._launcher = GameLauncher(rb, self)
                    self._launcher.game_launched.connect(self._on_game_launched)
                    self._launcher.game_exited.connect(self._on_game_exited)
                    self._launcher.launch_failed.connect(self._on_launch_failed)
            except Exception as e:
                print(f"[WARN] Could not initialize RetroBat: {e}")

    def _on_set_retrobat_path(self):
        """Let the user browse to their RetroBat folder."""
        path = QFileDialog.getExistingDirectory(
            self, "Select RetroBat Installation Folder",
            self._settings.retrobat_path or "D:/",
            QFileDialog.Option.ShowDirsOnly,
        )
        if not path:
            return
        rb = RetroBatInstall(Path(path))
        if not rb.is_valid():
            QMessageBox.warning(self, "Invalid Path",
                f"This doesn't look like a valid RetroBat installation:\n{path}\n\n"
                "Make sure emulationstation/emulatorLauncher.exe exists.")
            return
        self._retrobat = rb
        self._launcher = GameLauncher(rb, self)
        self._launcher.game_launched.connect(self._on_game_launched)
        self._launcher.game_exited.connect(self._on_game_exited)
        self._launcher.launch_failed.connect(self._on_launch_failed)
        self._settings.retrobat_path = path
        # Import SS credentials from RetroBat
        if self._settings.import_ss_credentials_from_retrobat():
            self._status_label.setText(
                f"RetroBat connected: {path}  ·  ScreenScraper credentials imported"
            )
        else:
            self._status_label.setText(f"RetroBat connected: {path}")
        self._settings.save()

        # If no library loaded yet, auto-load from RetroBat roms folder
        if not self._library._all_entries:
            roms = rb.roms_root
            if roms.exists():
                self._load_library(roms)

    # ── Launch ────────────────────────────────────────────────────────────────

    def _on_launch(self, entry: RomEntry):
        if not self._launcher:
            QMessageBox.information(self, "RetroBat Not Configured",
                "Set your RetroBat installation path first:\n"
                "  File → Set RetroBat Path…")
            return
        if not self._launcher.can_launch(entry):
            QMessageBox.warning(self, "Cannot Launch",
                f"ROM file not found:\n{entry.rom_path}")
            return
        self._launcher.launch(entry)

    def _on_game_launched(self, entry: RomEntry):
        self._status_label.setText(f"  ▶  Launched: {entry.name}  ({entry.system_full_name})")

    def _on_game_exited(self, entry: RomEntry, rc: int):
        self._status_label.setText(
            f"  ■  {entry.name} exited  ·  Plays: {entry.play_count}"
        )
        # Refresh detail panel to show updated play count
        if self._current_selection and entry in self._current_selection:
            self._detail.show_entry(entry)

    def _on_launch_failed(self, entry: RomEntry, msg: str):
        QMessageBox.critical(self, "Launch Failed",
            f"Could not launch {entry.name}:\n\n{msg}")

    # ── Rental collection sync ────────────────────────────────────────────────

    def _on_rent_toggled(self, entry, is_rented: bool):
        """Keep CollectionManager's Rented collection in sync with the rental shop."""
        if is_rented:
            self._collections.rent(entry)
        else:
            self._collections.return_game(entry)

    # ── Stats dashboard ───────────────────────────────────────────────────────

    def _on_show_stats(self):
        from .stats_dashboard import StatsDashboardDialog
        all_entries = self._library.get_all()
        if not all_entries:
            QMessageBox.information(self, "No Library",
                "Open a ROM library first.")
            return
        dlg = StatsDashboardDialog(all_entries, self)
        dlg.exec()

    # ── Missing artwork report ────────────────────────────────────────────────

    def _on_missing_artwork(self):
        all_entries = self._library.get_all()
        if not all_entries:
            QMessageBox.information(self, "No Library", "Open a ROM library first.")
            return
        from .missing_artwork_dialog import MissingArtworkDialog
        dlg = MissingArtworkDialog(all_entries, self)
        dlg.exec()
    ...
    def _build_menus(self):
        ...
        rb_act = QAction("Set RetroBat &Path…", self)
        rb_act.setToolTip("Point to your RetroBat installation folder")
        rb_act.triggered.connect(self._on_set_retrobat_path)
        file_menu.addAction(rb_act)
        ...
        scrape_act = QAction("🔍  &Scrape Artwork…", self); scrape_act.setShortcut("Ctrl+Shift+S")
        scrape_act.triggered.connect(lambda: self._on_scrape(self._current_selection or self._library.get_all()))
        edit_menu.addAction(scrape_act)
        ...
        stats_act = QAction("📊  Play Statistics…", self)
        stats_act.setShortcut("Ctrl+T")
        stats_act.triggered.connect(self._on_show_stats)
        tools_menu.addAction(stats_act)
        art_act = QAction("🖼  Missing Artwork Report…", self)
        art_act.triggered.connect(self._on_missing_artwork)
        tools_menu.addAction(art_act)
        ...
        tools_menu.addSeparator()
        emu_act = QAction("⚙  Emulator / Core Config…", self)
        emu_act.triggered.connect(lambda: self._on_emulator_config(
            self._current_selection[0]) if self._current_selection else
            QMessageBox.information(self, "Select a ROM",
                "Right-click a ROM or select one first."))
        tools_menu.addAction(emu_act)
    ...
    def _on_load_finished(self, entries):
        ...
        self._collections.set_all_entries(entries)
        self._sidebar.populate(self._library.get_systems(),
                               self._library.get_genres(),
                               self._library.get_years())
        # Populate collections section: Recent + Rented + custom
        coll_names = ["Recent", "Rented"] + self._collections.list_custom()
        self._sidebar.populate_collections(coll_names)
        ...
        self._library.filter_collection = None
        self._refresh_view()
    ...
    def _on_filter(self, filter_type: str, value):
        if filter_type == "system":
            ...
            self._library.filter_collection = value
            self._library.filter_system = None
            self._library.filter_genre = None
            self._library.filter_year = None
            self._library.filter_favorites_only = False
    ...
    def _do_refresh(self):
        # Collection filter overrides library filter
        coll_name = getattr(self._library, "filter_collection", None)
        if coll_name:
            entries = self._collections.collection_entries(coll_name)
        else:
        ...
        # Use grouped view when filtering by genre or year (not system, not collection)
        use_grouped = (
    ...
    def _show_context_menu(self, entries: List[RomEntry]):
        ...
        # Launch (single ROM)
        if n == 1:
            act_launch = menu.addAction(f"▶  Launch — {entries[0].name}")
            act_launch.setFont(_bold_font())
            act_launch.triggered.connect(lambda: self._on_launch(entries[0]))
            menu.addSeparator()
        ...
        # Scrape
        act_scrape = menu.addAction(f"🔍  Scrape Artwork — {label}")
        act_scrape.triggered.connect(lambda: self._on_scrape(entries))

        # Emulator config (single ROM / system)
        if n == 1 and self._retrobat:
            act_emu = menu.addAction("⚙  Emulator / Core Config…")
            act_emu.triggered.connect(lambda: self._on_emulator_config(entries[0]))
    ...
    def _on_emulator_config(self, entry: RomEntry):
        if not self._retrobat:
            QMessageBox.information(self, "RetroBat Not Configured",
                "Set your RetroBat path first via File → Set RetroBat Path…")
            return
        from .emulator_config_dialog import EmulatorConfigDialog
        dlg = EmulatorConfigDialog(self._retrobat, entry.system, entry, self)
        dlg.exec()

    # ── Scrape ────────────────────────────────────────────────────────────────

    def _on_scrape(self, entries: List[RomEntry]):
        if not entries:
            return
        try:
            from .scrape_dialog import ScrapeDialog
            # Pre-fill SS credentials from settings
            dlg = ScrapeDialog(entries, self._settings, self)
            dlg.exec()
        except Exception as e:
