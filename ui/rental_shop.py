...
class VirtualRentalShopPanel(QWidget):
    ...
    rent_toggled     = pyqtSignal(object, bool)  # (RomEntry, is_rented) — for CollectionManager
    ...
    def _on_rent_toggle(self, entry: RomEntry, want_rent: bool):
        ...
        self.rent_toggled.emit(entry, want_rent)
        self._update_header()
    ...
    def _on_return_all(self):
        ...
        if QMessageBox.question(
            ...
            return
        returned = [c.entry for c in self._cards if c.is_rented]
        self._state["rented_paths"] = []
        _save_state(self._state)
        for card in self._cards: card.set_rented(False)
        for entry in returned:
            self.rent_toggled.emit(entry, False)
        self._update_header()
