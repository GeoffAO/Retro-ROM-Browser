...
class SidebarPanel(QFrame):
    ...
    def populate_collections(self, collection_names: list):
        """
        Add or refresh the COLLECTIONS section at the top of the tree.
        Call this after populate() so it appears above systems.

        collection_names: list of str, e.g. ["Recent", "Rented", "My Picks"]
        The first four built-ins (All Games, Favorites) are shown as quick-filter
        buttons, so only show the rest here.
        """
        # Remove any existing collections section
        for i in range(self.tree.topLevelItemCount()):
            item = self.tree.topLevelItem(i)
            if isinstance(item, SectionHeader) and "COLLECTIONS" in item._base_label:
        ...
        coll_root = SectionHeader("COLLECTIONS")
        for name in collection_names:
            item = TagItem(name, 0, "collection", name)
            item.setForeground(0, QBrush(QColor("#70b0d0")))
            coll_root.addChild(item)
        # Insert at position 0 (above systems)
        self.tree.insertTopLevelItem(0, coll_root)
        coll_root.setExpanded(True)
